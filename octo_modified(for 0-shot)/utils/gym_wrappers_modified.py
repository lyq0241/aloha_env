...
#I cannot find metadata["proprio"], so I use this to normalize
    def normalize_without_metadata(self, data):
        mean = np.mean(data)
        std = np.std(data)
    ...
    def observation(self, obs):
        #to be deleted
        #print("metadata.keys", self.action_proprio_metadata["bridge_dataset"].keys())
        ...
        if "proprio" in self.action_proprio_metadata:
            ...
            #to be deleted
            obs["proprio"] = self.normalize(
                obs["proprio"], self.action_proprio_metadata["bridge_dataset"]["proprio"]
            )
        return obs
